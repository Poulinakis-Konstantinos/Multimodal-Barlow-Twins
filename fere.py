    ...
    def forward(self, mods, m_ra=1, pad_len=None):
        ...
        if self.training:
            ...
            if len(self.beta_mod) == 1 or self.single_pick:
                ...
                print('Aug modal mask (216):', aug_modal_mask.size())
            ...
            for i_modal, p_modal in enumerate(self.p_t_mod):
                print('LOOP - Mods size:', mods.size())
...
if __name__ == "__main__":
    import torch
    fere = FeRe_modal(p_t_mod=[.3, .3, 0.3],p_modal=[1.0, 0.1, 0.5]) # Why does it still augment despite prob=0?
